...
export function renderAttack(cell, board, boardElement, shipWrappers, reports) {
  ...
  if (attacked instanceof Ship) {
    ...
    reportAttack(
      reports,
      `Hit enemy ${shipName} at ${cell.dataset.row}${numberToLetter(cell.dataset.col)}`,
    );
    ...
    reportAttack(
      reports,
      `Missed an attack at ${cell.dataset.row}${numberToLetter(cell.dataset.col)}`,
    );
  }
}

function handleSunkShip(
  cell,
  boardElement,
  shipWrappers,
  reports,
  ...
  if (!cell.parentElement.classList.contains("ship")) {
    const cells = Gameboard.getCells(
      attacked.location.start,
      attacked.length,
      attacked.location.orientation,
    );
    for (let [row, col] of cells) {
      const shipCell = boardElement.querySelector(
        `button.disabled.hit[data-row="${row}"][data-col="${col}"]`,
      );
  ...
  for (let ship in shipElelemts) {
    ...
    if (playerShips[ship].location.orientation === "vertical") {
      shipElelemts[ship].setAttribute(
        "style",
        `grid-area: ${ShipLocation.start[0] + 1} / ${ShipLocation.start[1] + 1} / span ${playerShips[ship].length} / ${ShipLocation.start[1] + 1}; grid-template-rows: repeat(${playerShips[ship].length}, 1fr)`,
      );
    } else {
      shipElelemts[ship].setAttribute(
        "style",
        `grid-area: ${ShipLocation.start[0] + 1} / ${ShipLocation.start[1] + 1} / ${ShipLocation.start[0] + 1} / span ${playerShips[ship].length}; grid-template-columns: repeat(${playerShips[ship].length}, 1fr)`,
      );
    }
  }
  return shipElelemts;
